...
@bot.command(name='hi', help='Responds with hi')
async def hello(ctx):
...
async def google_search(ctx, *args):
    try:
        if len(args) != 0:
            # handle if user enter search keyword without qoutes
            keyword = ' '.join(args)
            ...
            await ctx.send("Please provide keywords to search.")
...
async def recent_searchs(ctx, *args):
    try:
        if len(args) != 0:
            # handle if user enter search keyword without qoutes
            keyword = ' '.join(args)
            ...
            await ctx.send("Please provide a keyword to check in search history.")

    except Exception as e:
        print("error in search history",e)
